...
(function($) {
    $.fn.panelMultiSelect = function(options) {
        ...
        var defaults = {
            ...
            notSelectedElementCss: {
                color: 'black',
                background: 'white',
                border: ''
            },
            selectedCountColor: '#608001',
            selectedElementStyle: 'color: white; background: #009970; border: ',
        ...
        function renderUiFromData() {
            ...
            $(dataSource).each(function() {
                ...
                buttonElementListString += halfBeforeStringBtn_1 + notSelectedElementStyle + halfBeforeStringBtn_2 + '" data-key ="' + keyData + '">' + valueData + '</button>';
                selectedButtonElementListString += halfBeforeStringBtn_1 + selectedElementStyle + halfBeforeStringBtn_2 + '" data-key ="' + keyData + '">' + valueData + '</button>'
            });
            ...
            const headerMultiSelectString = '<div class="' + settings.tvSelectHeaderClassName + '">' + '<div class="align-height-center-left" style="float: left"><span style="display: flex">' + '<div> <span class="btn-xs-custom" id="' + UN_SELECT_ALL_BTN_ID + '">UnSelect All</span> </input>' + '</div></div>' + '<div style="float: right" class="align-height-center-right">' + '<span class="input-group search-input-group"><input class="input-search" id="' + SEARCH_INPUT_ID + '" placeholder="Typing keyword..." type="text"/><span id="' + SEARCH_CLEAR_ID + '" class="clear-search">x</span></span>' + '</div><div class="clearfix"></div></div>';
            const footerMultiSelectString = '<div class="' + settings.tvSelectFooterClassName + '">' + '<div style="float: left"  class="footer-height-center"><span class="active-mode hide" id="' + SEARCH_MODE_ID + '">Search Mode</span><div id="' + GROUP_MODE_ID + '"></div></div>' + '<div style="float: right" class="footer-height-center">' + '<span>Selected: <span style="color: ' + settings.selectedCountColor + '" class="number-content" id="selectedCountSpan">0' + '</span><span class="space">&nbsp;</span><span class="blue-span">/</span><span class="space">&nbsp;</span><span class="number-content blue-smooth">' + formatNumberString(COUNT_TOTAL_ELEMENT.toString()) + '</span></span><span class="clearfix"></span></div>' + '</div></div></div>';
            const contentMultiSelectString = '<div style="height: ' + contentHeight + 'px; width: ' + ($(elementReference).width() - 5) + 'px" class="' + settings.selectPanelContentWrapperClassName + '" id="' + settings.selectPanelContentWrapperClassName + '"><div id="' + CHECKBOX_WRAPPER_ID + '" class="' + settings.checkboxDivWrapperClassName + '" style="min-height:' + ($(elementReference).height() - 10) + 'px"></div><div id="' + BUTTON_WRAPPER_ID + '" style="float:left;width: ' + buttonWrapperWidth + 'px;">' + buttonElementListString + '</div><div class="clearfix"></div></div>';
            $(elementReference).append('<div id="' + SELECT_PANEL_WRAPPER_ID + '">' + headerMultiSelectString + contentMultiSelectString + footerMultiSelectString + '</div>');
        ...
        function setupCheckboxRowList() {
            ...
            for (let index = 0; index < rowSize; index++) {
                checkboxElementListString += '<div class="' + settings.checkboxClassName + '" style="height: ' + checkboxHeight + 'px"><input type="checkbox" data-index ="' + index + '"/></div>'
            }
        ...
        function setupSelectedButton(buttonElement) {
            ...
            selectedMap.set(buttonElement.index, {
                key: buttonElement.key,
                value: buttonElement.value
            });
            buttonElement.checked = !0
        }
        ...
        function setupShowMultiSelectMode() {
            $('#' + MULTI_SELECT_MODE_ID).on('click', function() {
                ...
                $('#' + SEARCH_INPUT_ID).prop('disabled', !1);
        ...
        function setupShowPreviewMode() {
            $('#' + PREVIEW_MODE_ID).on('click', function() {
                ...
                if (selectedMap.size !== 0) {
                    previewDivString = '<div class="previewDiv" id="' + PREVIEW_DIV_ID + '">' + buttonListString + '</div>'
                } else {
                    previewDivString = '<div class="previewDiv loading-div" id="' + PREVIEW_DIV_ID + '">' + 'No selected data to preview...' + '</div>'
                }
        ...
        function setupEventSearching() {
            ...
            function makeButtonString(isSelected, index, value) {
                return halfBeforeStringBtn_1 + (isSelected ? selectedElementStyle : notSelectedElementStyle) + halfBeforeStringBtn_2 + '"data-selected ="' + (isSelected ? 1 : 0) + '"  data-index ="' + index + '">' + value + '</button>'
            }
            $('#' + SELECT_ALL_BTN_ID).css("pointer-events", "none");
            $('#' + UN_SELECT_ALL_BTN_ID).css("pointer-events", "none");
            $('#' + SHOW_ONLY_SELECTED_CB_ID).prop('disabled', !0);
            $(panelContentChildren).each(function() {
                $(this).hide()
            });
            ...
            let searchContent = '<div class="search-header"><div class="search-header-right"><button id="' + EXIST_SEARCH_BTN_ID + '" class="btn-xs-custom">Exit Search Mode</button></div><div class="search-header-left">' + (searchTxt.length !== 0 ? '- ' + '<span style="font-weight: bold">' + countResult + '</span>&nbsp;results for keyword "' + searchTxt + '"' : '') + '</div><div class="clearfix"></div></div>' + '<div id="' + SEARCH_DIV_CONTENT + '">' + buttonListString + '</div>';
            if (!searching) {
                searching = !0;
            ...
            $('#' + EXIST_SEARCH_BTN_ID).on('click', function() {
                ...
                $('#' + SELECT_ALL_BTN_ID).css("pointer-events", "auto");
                $('#' + UN_SELECT_ALL_BTN_ID).css("pointer-events", "auto");
                $('#' + SHOW_ONLY_SELECTED_CB_ID).prop('disabled', !1);
                clearSearch()
            });
        ...
        function showLoading() {
            $(elementReference).append('<div class="loading-div" id="' + ELEMENT_ID + '_loading">Initializing, please wait...</div>')
        }
